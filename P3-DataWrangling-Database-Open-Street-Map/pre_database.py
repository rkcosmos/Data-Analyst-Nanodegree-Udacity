...
def shape_element(element, node_attr_fields=NODE_FIELDS, way_attr_fields=WAY_FIELDS,
    ...
    find_before = re.compile(r"^[^:]*")
    find_after = re.compile(r"(?::)(.*)")
    ...
    if element.tag == 'node':
        ...
        for child in element.findall("tag"):
            ...
            if ":" in child.get("k"):
                ...
                att_dict["type"] = "regular"
                att_dict["key"] = child.get("k")
        return {'node': node_attribs, 'node_tags': tags}
    elif element.tag == 'way':
        for item in WAY_FIELDS:
            way_attribs[item] = element.get(item)
        for i,child in enumerate(element.findall("nd")):
            att_dict = {}
            way_nodes.append(att_dict)
            att_dict["id"] = element.get("id")
            att_dict["node_id"] = child.get("ref")
            att_dict["position"] = i
        for child in element.findall("tag"):
            att_dict = {}
            tags.append(att_dict)
